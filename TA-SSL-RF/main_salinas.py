from util import setup_logger, Splicing_result_Salinas, fix_random_seeds, aligned_sample
from Dataset.Dataset_swin_salinas import mymodel, getimg, train_pca
...
def train(args, result_train, label_train, number=2000):
    ...
    X, Y = aligned_sample(args, result, label, number=number, rise=0.7, ft_number=5100)
...
def val(args, grid, result_val, label_val, number=1000):  # 对数据的各个类别分别进行采样from sklearn.preprocessing import StandardScaler
    ...
    X, Y = aligned_sample(args, result, label, rise=0.3, number=number, ft_number=5100)  # 对每个类别分别进行2000次采样 ,并且打乱顺序
    X = np.array(X)
    for i in range(21):
        if i == 0:
            continue
...
def test(args, grid, result_test, label_test):
    ...
    Splicing_result_Salinas(args, out.reshape(n * h * w), Y_val)  # 还原出结果图
    return yfit
...
result_train, label_train = getimg(args, logger, loaders="test", number=3, models=models)
...
grid = train(args, result_train, label_train, number=3000)  # 在均衡样本上进行训练
val(args, grid, result_train, label_train, number=3000)  # 在均衡的部分样本上测试
