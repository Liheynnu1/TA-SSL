...
class Dataset(Dataset):
    def __init__(self,
        ...
        self.image1s = np.array(Image.open(image1_paths), dtype=np.uint8)[::20, ::20]
        self.image1s = Image.fromarray(np.array(add_zero(args, self.image1s), dtype=np.uint8))
        # self.image = [Image.open(os.path.join(image2_paths, path)) for path in image2s]
        data = gdal.Open(image2_paths)
        tmp_img = data.ReadAsArray()  # 将数据转为数组
        image2s = tmp_img.transpose(1, 2, 0)  # 由波段、行、列——>行、列、波段
        image2s = standardization_org(image2s)
        self.HSI = Composite_feature_map(self.args, [image2s])
        self.labels = np.array(Image.open(label_paths))
        self.labels = Image.fromarray(np.array(add_zero(args, self.labels[::2, ::2], no_c=True), dtype=np.uint8))
    ...
    def __getitem__(self, index):
        if self.loaders == 'val':  # 如果是测试数据就跳过加载过的加载数据
            ...
            assert index < (54 * 216)  # 加载的数据过多

        if self.loaders != 'test':
            index = self.all_index[index]
        out_crop = []
        w, h = np.shape(self.labels)[-2:]
        Row = h // self.img_size - 1
        column = w // self.img_size - 1

        # 计算对应的标签块
        x1 = index % Row * self.img_size
        y1 = index // Row * self.img_size
        x2 = x1 + 224 + self.margin*2
        y2 = y1 + 224 + self.margin*2
...
class mymodel(torch.nn.Module):
    def __init__(self,
        ...
        if args.model == 'swin':
            ...
            self.backbone = self.prepare_vitmodel(args.checkpoint2_path, 'vit_base_patch16')



        self.maxpool = nn.AvgPool2d(kernel_size=20, stride=20, padding=1)
    ...
    def forward(self, img):
        ...
        if self.args.model == 'vit':
            ...
            # out = self.feature_scaling([out.reshape((out.shape[0],out.shape[1], int(out.shape[2]**0.5), int(out.shape[2]**0.5)))])
        else:
            # out = unpatchify_swin(out)
            out = self.online_projector(out)
            out = self.feature_scaling(out)

        if self.maxpool_10 and self.addfeature:  # RGB图像数据
            pass
        elif self.addfeature:  # 高光谱数据
            out = torch.cat([out, img], dim=1)
        elif self.maxpool_10 and not self.addfeature:
            pass
...
def get_feature(args, model, image):
    out = np.zeros((image.shape[0], args.image_size, args.image_size, 768 if args.model == 'vit' else 1024))
    idx = int(args.piece_size/2)
    stride = int(args.piece_size)
    for i in range(0, idx*2, 2):
        for j in range(0, idx*2, 2):
            a = out[:, ::stride+i, ::stride+j, :]
            in_img = image[:, :, i:224+i, j:224+j]
            a2 = model(in_img).detach().to('cpu').reshape((image.shape[0],
                                                           int(args.image_size/args.piece_size),
                                                           int(args.image_size/args.piece_size),
                                                           -1))
            out[:, i::stride,     j::stride, :] = a2
            out[:, i::stride,   j+1::stride, :] = a2
            out[:, i+1::stride,   j::stride, :] = a2
            out[:, i+1::stride, j+1::stride, :] = a2
    out = torch.tensor(np.transpose(out, (0, 3, 1, 2)))
    return out
...
def getimg(args, logger, loaders="train", number=20, models =None, position_offset=0):
    ...
    if loaders == "train":
        ...
        valDataset = Dataset(args, test_image1_paths, test_image2_paths, test_label_paths,
                             len=number, loaders=loaders, position_offset=position_offset)
        val_loader = DataLoader(valDataset, batch_size=args.batch_size, shuffle=False, num_workers=0, pin_memory=True,
                                drop_last=False)
        loader = val_loader
    elif loaders == "test":
        testDataset = Dataset(args, test_image1_paths, test_image2_paths, test_label_paths,
                              len=number, loaders=loaders)
    ...
    for data in tqdm(loader):
        ...
        targets.append(target[..., margin:img_size+margin, margin:img_size+margin])
