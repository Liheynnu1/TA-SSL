...
class T2T_module(nn.Module):
    ...
    def forward(self, x):
        # step0: soft split
        x = self.soft_split0(x).transpose(1, 2)  # 64,3136,147

        # iteration1: re-structurization/reconstruction
        x = self.attention1(x)  # 64,3136,64
        B, new_HW, C = x.shape
        x = x.transpose(1,2).reshape(B, C, int(np.sqrt(new_HW)), int(np.sqrt(new_HW)))  # 64,64,56,56
        # iteration1: soft split
        x = self.soft_split1(x).transpose(1, 2)  # 64,784,576

        # iteration2: re-structurization/reconstruction
        x = self.attention2(x)  # 64,784,64
        B, new_HW, C = x.shape
        x = x.transpose(1, 2).reshape(B, C, int(np.sqrt(new_HW)), int(np.sqrt(new_HW)))  # 64,64,28,28
        # iteration2: soft split
        x = self.soft_split2(x).transpose(1, 2)  # 64,196,576

        # final tokens
        x = self.project(x)  # 64,196,512    64,14,14,512
...
class T2T_ViT(nn.Module):
    ...
    def forward_features(self, x):
        B = x.shape[0]
        x = self.tokens_to_token(x)#64，196，512

        cls_tokens = self.cls_token.expand(B, -1, -1)
        x = torch.cat((cls_tokens, x), dim=1)  # 64，197，512
        x = x + self.pos_embed  # 64，197，512
        x = self.pos_drop(x)  # 64，197，512
