...
from .backbone_wrapper import MaskResNet, MaskSwin, Maskvit, Mask_T2tvit #MaskInterImage,
...
class PretrainModel(nn.Module):
    def __init__(self,
        ...
        if config.backbone == "swin":
            self.online_encoder = MaskSwin(config.mask_stage,
                                           replace=config.residual,
                                           out_indices=(2,3),
                                           in_channels=config.in_chans
                                           )
            self.branch_encoder = SwinTransformer(arch="t",
                                                  in_channels=config.in_chans,
                                                  # patch_size=8,
                                                  # window_size=4,
                                                  drop_rate=0., drop_path_rate=0.)
            self.mim2org = MIMHead(in_channels=config.in_channels,
                                   in_chans=config.in_chans)
        elif config.backbone == "vit":
            self.online_encoder = Maskvit(config.mask_stage,
                                          config.out_stage,
                                          config.backbone,
                                          in_channels=768,
                                          depth="B",
                                          replace=config.residual)
            self.branch_encoder = vit_base_patch16()
            self.mim2org = MIMHead(in_channels=config.in_channels, encoder_stride=8)
        elif config.backbone == "T2T":
            self.online_encoder = Mask_T2tvit(config.mask_stage,
                                              config.out_stage,
                                              config.backbone,
            ...
            self.branch_encoder = t2t_vit_14()
            self.mim2org = MIMHead(in_channels=config.in_channels, encoder_stride=16)
        else:
    ...
    def mim_forward(self, imgBatch: Union[Dict, List], device=torch.device("cpu")) -> Dict:
        ...
        img_rec_org = self.mim2org(mask_embedding_org)  # 32, 768, 14, 14   # 32. 192. 7. 7
        loss_rec = self.mim_loss(org_image, img_rec_org, mim_mask) # 16.3.224.224 16.56.56
        ...
        out = dict(loss_rec=loss_rec,
                   ...
                   view_embedding=[view_embedding_org, view_embedding_view],  # 教师网络
                   mask=[mim_mask.reshape(mim_mask.size(0), -1), view_mask.reshape(mim_mask.size(0), -1)],
                   locations=locations)
...
class MoCoBased(PretrainModel):
    ...
    #     loss_rec, mask_embedding, view_embedding, mask, locations = backbone_dict["loss_rec"], \
    #                                                                 backbone_dict["mask_embedding"], \
    #                                                                 backbone_dict["view_embedding"], \
    #                                                                 backbone_dict["mask"], \
    #                                                                 backbone_dict["locations"]
    #
    #     mim_mask, view_mask = mask
    #
    ...
    #
    #         global_loss = mocov2plus_loss_func(proj_embedding_org[-1], view_proj_embedding_view[-1], queue,
    #                                            self.temperature)
    ...
    #         mask_embedding_org = rearrange(mask_embedding_org, " B C H W -> B (H W) C")  # 16,49,192
    ...
    #         # a = mim_mask.to(torch.bool)
    #         b,c1,c2=mim_mask.shape[0], location_1.shape[-1],mask_embedding_org.shape[-1]
    #         maps_1_filtered, maps_1_nn = position_match(
    #             location_1[mim_mask.to(torch.bool), :].reshape(b, -1, c1),
    #             location_2[view_mask.to(torch.bool), :].reshape(b, -1, c1),
    #             mask_embedding_org[mim_mask.to(torch.bool),:].reshape(b, -1, c2),
    #             view_embedding_view[view_mask.to(torch.bool),:].reshape(b, -1, c2),
    #             num_matches=self.num_matches
    #         )
    #
    #         maps_2_filtered, maps_2_nn = position_match(  # 16.20.192
    #             location_2[mim_mask.to(torch.bool),:].reshape(b, -1, c1),
    #             location_1[view_mask.to(torch.bool),:].reshape(b, -1, c1),
    #             mask_embedding_view[mim_mask.to(torch.bool),:].reshape(b, -1, c2),  # 16.49.192
    #             view_embedding_org[view_mask.to(torch.bool),:].reshape(b, -1, c2),
    #             num_matches=self.num_matches
    #         )
    #
    #         # maps_1_filtered, maps_1_nn = position_match(
    #         #     location_1[mim_mask.to(torch.bool) == 0, :].reshape(b, -1, c1),
    #         #     location_2[view_mask.to(torch.bool) == 0, :].reshape(b, -1, c1),
    #         #     mask_embedding_org[mim_mask.to(torch.bool) == 0, :].reshape(b, -1, c2),
    #         #     view_embedding_view[view_mask.to(torch.bool) == 0, :].reshape(b, -1, c2),
    #         #     num_matches=self.num_matches
    #         # )
    #         #
    #         # maps_2_filtered, maps_2_nn = position_match(  # 16.20.192
    #         #     location_2[mim_mask.to(torch.bool) == 0,:].reshape(b, -1, c1),
    #         #     location_1[view_mask.to(torch.bool) == 0,:].reshape(b, -1, c1),
    #         #     mask_embedding_view[mim_mask.to(torch.bool) == 0,:].reshape(b, -1, c2),  # 16.49.192
    #         #     view_embedding_org[view_mask.to(torch.bool) == 0,:].reshape(b, -1, c2),
    #         #     num_matches=self.num_matches
    #         # )
    #
    #         # maps_1_filtered, maps_1_nn = position_match(
    ...
    #         #     num_matches=self.num_matches
    #         # )
    #         mask_proj_1 = self.class_projector(maps_1_filtered.permute(0, 2, 1))  # 4.2048.15
    ...
    def forward(self, imgBatch: List, device=torch.device("cpu")) -> Dict:
        ...
        if self.global_branch or self.model_warmup:
            ...
            self._dequeue_and_enqueue(gather(view_proj_embedding_org[-1]))
            # Mocov2网络的损失函数
            # global_loss -= global_loss  Mocov2
            total_loss += global_loss
            out.update(dict(global_loss=global_loss))
        # 教师网络
        if not self.model_warmup:
